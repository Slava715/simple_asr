...
class NumberExtractor(Extractor):
        ...
        if text:
            ...
            for match in matches:
                ...
                if match.fact.multiplier:
                    if str(match.fact.int)[-1] == '_':
                        num = str(match.fact.int) + " " + str(match.fact.multiplier)
                        mask.append(1)
                        mask.append(1)
                    else:
                        num = str(match.fact.int * match.fact.multiplier)
                        mask.append(2)
                else:
                    num = str(match.fact.int)
                    mask.append(1)
    ...
    def regroup_after_replace(self, text):
        ...
        if text:
            ...
            for i, n in enumerate(nums[1:]):
                if is_summable(new_nums[-1], n) and new_nums[-1] % 10 == 0:
                    print(new_nums[-1], n)
                    ...
                    if new_nums[-1] > 0:
                        mp = get_multiplexer(new_nums[-1]) // 10
                    else:
                        mp = 1
                    
                    m = get_multiplexer(n)
                    extracted = int(n / m)
                    new_nums[-1] = (new_nums[-1] + extracted * mp) * m // mp
                    
                    print(mp, m, extracted, new_nums)
    ...
    def replace_num(self, text, apply_regrouping=False):
        ret_text, mask = self.replace(text, apply_regrouping)
        return ret_text.replace("_", "")
